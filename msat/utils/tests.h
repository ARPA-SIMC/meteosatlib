 ...
 */
extern const msat::tests::LocationInfo msat_test_location_info;
...
#define wassert(...) \
    do { try { \
        __VA_ARGS__ ; \
    } catch (msat::tests::TestFailed& e1) { \
        e1.add_stack_info(__FILE__, __LINE__, #__VA_ARGS__, msat_test_location_info); \
        throw; \
    } catch (std::exception& e2) { \
        throw msat::tests::TestFailed(e2, __FILE__, __LINE__, #__VA_ARGS__, msat_test_location_info); \
    } } while(0)
...
#define wcallchecked(func) \
    [&]() { try { \
        return func; \
    } catch (msat::tests::TestFailed& e) { \
        e.add_stack_info(__FILE__, __LINE__, #func, msat_test_location_info); \
        throw; \

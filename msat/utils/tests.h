...
#define wassert(...) \
    do { try { \
        ...
        e.add_stack_info(__FILE__, __LINE__, #__VA_ARGS__, msat_test_location_info); \
        throw; \
    } catch (std::exception& e) { \
        throw msat::tests::TestFailed(e, __FILE__, __LINE__, #__VA_ARGS__, msat_test_location_info); \
    } } while(0)

...
$builddep -y fedora/SPECS/meteosatlib.spec

if [[ $image =~ ^fedora: || $image =~ ^centos: ]]
then
    pkgname=meteosatlib-master
    mkdir -p ~/rpmbuild/{BUILD,BUILDROOT,RPMS,SOURCES,SPECS,SRPMS}
    cp fedora/SPECS/meteosatlib.spec ~/rpmbuild/SPECS/meteosatlib.spec
    git archive --prefix=$pkgname/ --format=tar HEAD | gzip -c > ~/rpmbuild/SOURCES/$pkgname.tar.gz
    rpmbuild -ba --define "srcarchivename $pkgname" ~/rpmbuild/SPECS/meteosatlib.spec
else
    autoreconf -ifv
    ./configure
    make check

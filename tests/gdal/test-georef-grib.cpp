...
} test("gdal_georef_grib", "MsatGRIB", TESTFILE);
...
add_method("subsatellite", [](Fixture& f) {
    ...
    wassert(actual(px).almost_equal(0, 5));
    wassert(actual(py).almost_equal(0, 5));
    ...
    wassert(actual(lat).almost_equal(0, 5));
    wassert(actual(lon).almost_equal(0, 5));
